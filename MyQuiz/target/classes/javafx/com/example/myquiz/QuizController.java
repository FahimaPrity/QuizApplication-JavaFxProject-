...
public class QuizController {
    ...
    public void opt1clicked(ActionEvent event) {
        ...
        if (counter == 9) {
            ...
            try {
                ...
                Scene scoreScene = new Scene(scoreLoader.load());

                // Pass correct and wrong counts to the ScoreController
                ScoreController scoreController = scoreLoader.getController();
                scoreController.initialize(correctCount, wrongCount);

                scoreScene.setFill(Color.TRANSPARENT);
    ...
    private void onSubmitClicked() {
        // Implement logic to calculate and display the quiz score
    ...
    private void nextClicked(ActionEvent event) {
        if (counter < 9) { // Ensure counter does not exceed the maximum question index
            counter++;
            loadQuestions();
        }
    }
    ...
    private void prevClicked(ActionEvent event) {
        if (counter > 0) { // Ensure counter does not go below the minimum question index
            counter--;
            loadQuestions();
        }
    }
